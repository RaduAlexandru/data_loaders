...
void DataLoaderShapeNetPartSeg::init_data_reading(){
    ...
    fs::path labels_file = fs::path(m_dataset_path).parent_path()/ "colorscheme_and_labels" / m_restrict_to_object/"labels.txt";
    fs::path colorscheme_file = fs::path(m_dataset_path).parent_path() / "colorscheme_and_labels" / m_restrict_to_object/"color_scheme.txt";
    // fs::path frequency_file = fs::path(m_dataset_path).parent_path() / "colorscheme_and_labels" / m_restrict_to_object/"frequency.txt";
    fs::path frequency_file = fs::path(m_dataset_path).parent_path() / "colorscheme_and_labels" / m_restrict_to_object/"frequency_uniform.txt";
    int unlabeled_idx=0;
