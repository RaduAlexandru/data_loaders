...
Mesh Frame::backproject_depth(){
    ...
    cloud.apply_transform(tf_world_cam);
...
Mesh Frame::assign_color(Mesh& cloud){
    ...
    cloud.C.resize(cloud.V.rows(), cloud.V.cols());
    cloud.C.setZero();
    ...
    for (int i = 0; i < cloud.V.rows(); i++) {
        ...
        if(V_transformed(i,1)<0 || V_transformed(i,1)>rgb_32f.rows || V_transformed(i,0)<0 || V_transformed(i,0)>rgb_32f.cols){
            continue;
        }

        // VLOG(1) << "accessing at y,x" << V_transformed(i,1) << " " << V_transformed(i,0);
        // cloud.C(i,0)=rgb_32f.at<cv::Vec3f>(V_transformed(i,1), V_transformed(i,0)) [ 0 ];
        // cloud.C(i,1)=rgb_32f.at<cv::Vec3f>(V_transformed(i,1), V_transformed(i,0)) [ 1 ];
        // cloud.C(i,2)=rgb_32f.at<cv::Vec3f>(V_transformed(i,1), V_transformed(i,0)) [ 2 ];
    }
